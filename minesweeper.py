...
class Minesweeper:
        ...
        for (label, n) in zip(upper_alphabet_string[:self.height], range(self.height)):
            ...
            for m in range(self.width):
                if self.board[n][m] == 0:  # closed
                    ...
                    if self.bombs[n][m] == 0:  # if the neighboring cells are not bombs
                        print('   ', end='')
                    elif self.bombs[n][m] == -1:  # if bombs
                        print('*  ', end='')
                    else:  # if it has bombs around
    ...
    def start_game(self):
        ...
        while True:
            ...
            try:
                # ord() is used to convert alphabet to number
